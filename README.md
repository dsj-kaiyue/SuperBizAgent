...
方法一： 手动启动
```bash
1.先在 Linux 虚拟机启动向量数据库
ssh root@192.168.17.129 "cd /home/kaiyue/code/SuperBizAgent-release-2026-01-02 && sudo docker compose -f vector-database.yml up -d"
...
方法二：一键启动
```bash
make init  # 会通过 SSH 启动虚拟机 Docker，并上传运维文档到向量库
```
