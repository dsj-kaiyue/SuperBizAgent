...
VM_IP = 192.168.17.129
VM_USER = root
VM_PROJECT_DIR = /home/kaiyue/code/SuperBizAgent-release-2026-01-02
REMOTE_SSH = ssh $(VM_USER)@$(VM_IP)
REMOTE_COMPOSE_CMD = cd $(VM_PROJECT_DIR) && docker compose -f $(DOCKER_COMPOSE_FILE)
...
init:
	...
	@echo "   Attu (Web UI): http://$(VM_IP):8000"
	@echo ""
...
up:
	@echo "$(YELLOW)🐳 检查虚拟机 Docker 容器状态...$(NC)"
	@if ! $(REMOTE_SSH) "test -f $(VM_PROJECT_DIR)/$(DOCKER_COMPOSE_FILE)"; then \
		echo "$(RED)❌ 虚拟机中未找到 Compose 文件: $(VM_PROJECT_DIR)/$(DOCKER_COMPOSE_FILE)$(NC)"; \
		exit 1; \
	fi
	@if $(REMOTE_SSH) "docker ps --format '{{.Names}}' | grep -q '^$(MILVUS_CONTAINER)$$'"; then \
		echo "$(GREEN)✅ Milvus 容器已经在虚拟机运行中$(NC)"; \
		echo "$(YELLOW)📋 虚拟机当前运行的容器:$(NC)"; \
		$(REMOTE_SSH) "docker ps --filter 'name=milvus' --format 'table {{.Names}}\t{{.Status}}\t{{.Ports}}'"; \
	else \
		echo "$(YELLOW)🚀 在虚拟机启动 Docker Compose...$(NC)"; \
		$(REMOTE_SSH) "$(REMOTE_COMPOSE_CMD) up -d"; \
		echo ""; \
		...
		if $(REMOTE_SSH) "docker ps --format '{{.Names}}' | grep -q '^$(MILVUS_CONTAINER)$$'"; then \
			...
			echo ""; \
			echo "$(GREEN)📋 虚拟机运行中的容器:$(NC)"; \
			$(REMOTE_SSH) "docker ps --filter 'name=milvus' --format 'table {{.Names}}\t{{.Status}}\t{{.Ports}}'"; \
			echo ""; \
			echo "$(GREEN)🌐 服务访问地址:$(NC)"; \
			echo "   Milvus: $(VM_IP):19530"; \
			echo "   Attu (Web UI): http://$(VM_IP):8000"; \
			echo "   MinIO: http://$(VM_IP):9001 (admin/minioadmin)"; \
		else \
			echo "$(RED)❌ 容器启动失败，请检查日志: ssh $(VM_USER)@$(VM_IP) \"$(REMOTE_COMPOSE_CMD) logs\"$(NC)"; \
			exit 1; \
		fi; \
	fi
...
# 停止 Docker Compose
down:
	@echo "$(YELLOW)🛑 停止虚拟机 Docker Compose...$(NC)"
	@if $(REMOTE_SSH) "docker ps --format '{{.Names}}' | grep -q 'milvus'"; then \
		$(REMOTE_SSH) "$(REMOTE_COMPOSE_CMD) down"; \
		echo "$(GREEN)✅ 虚拟机 Docker Compose 已停止$(NC)"; \
	else \
		echo "$(YELLOW)⚠️  虚拟机没有运行中的 Milvus 容器$(NC)"; \
	fi

# 查看 Docker 容器状态
status:
	@echo "$(YELLOW)📊 虚拟机 Docker 容器状态:$(NC)"
	@echo ""
	@if $(REMOTE_SSH) "docker ps -a --format '{{.Names}}' | grep -q 'milvus'"; then \
		$(REMOTE_SSH) "docker ps -a --filter 'name=milvus' --format 'table {{.Names}}\t{{.Status}}\t{{.Ports}}'"; \
		echo ""; \
		running=$$($(REMOTE_SSH) "docker ps --filter 'name=milvus' --format '{{.Names}}' | wc -l" | tr -d ' '); \
		total=$$($(REMOTE_SSH) "docker ps -a --filter 'name=milvus' --format '{{.Names}}' | wc -l" | tr -d ' '); \
